...
parser.add_argument(
    ...
    type=int,
    default=64,
    help="Resize DVS frames to this spatial size.",
)
parser.add_argument(
    "--train-split-path",
    type=str,
...
parser.add_argument(
    "--pair-batches",
    action="store_true",
    help="Each step: concatenate B/2 clean + B/2 IPMix images into one batch of size B (requires even -b)."
)
...
def _force_pil_rgb_size(img, target_size):
    """
    Make sure image is PIL RGB and has the same size as the clean image.
    target_size is PIL format: (W, H).
    """
...
def ipmix(image: Image.Image, mixing_pic: Image.Image, preprocess):
    ...
    # Tiny ImageNet target size, e.g. (64, 64)
    target_size = image.size

    # Make clean and mixing image consistent
    image = _force_pil_rgb_size(image, target_size)
    mixing_pic = _force_pil_rgb_size(mixing_pic, target_size)
    ...
    for i in range(GLOBAL_ARGS.k):
        ...
        if mixing_ways == "P":
            for _ in range(np.random.randint(GLOBAL_ARGS.t + 1)):
                ...
                # IMPORTANT: patch_mixing may return 32x32 from CIFAR-style code
                mixed = _force_pil_rgb_size(mixed, target_size)

        else:
            for _ in range(np.random.randint(GLOBAL_ARGS.t + 1)):
                mixed = augment_input(mixed)

                # Some augmentations may also change size
                mixed = _force_pil_rgb_size(mixed, target_size)

        mixed_tensor = preprocess(mixed)

        # Final safety check
        if mixed_tensor.shape != base.shape:
            mixed_tensor = F.interpolate(
                mixed_tensor.unsqueeze(0),
                size=base.shape[-2:],
                mode="bilinear",
                align_corners=False,
            ).squeeze(0)

        mix += ws[i] * mixed_tensor

    mix_result = (1 - m) * base + m * mix
    return mix_result


class IPMixDataset(torch.utils.data.Dataset):
    """Dataset wrapper to perform IPMix before model / spike encoder."""
    def __init__(self, dataset, mixing_set, preprocess, no_jsd=False, basic_transform=None):
    ...
    def __getitem__(self, i):
        ...
        # Basic augmentation first, still on PIL image
...
class DVSIPMixDataset(torch.utils.data.Dataset):
    """
    IPMix-style wrapper for DVS frame tensors.
    Works with samples shaped [T, C, H, W], usually C=2 for DVS polarity frames.
    Keeps clean + two mixed views when no_jsd=False, same as original IPMix.
    """
    def __init__(self, dataset, mixing_set, no_jsd=False):
        self.dataset = dataset
        self.mixing_set = mixing_set
        self.no_jsd = no_jsd
    ...
    def _to_tensor(self, x):
        ...
        # Some DVS datasets return [C, T, H, W], convert to [T, C, H, W] if needed
        if x.ndim == 4 and x.shape[0] in (1, 2, 3) and x.shape[1] == GLOBAL_ARGS.time_steps:
            x = x.permute(1, 0, 2, 3).contiguous()

        return x

    def _mixing_image_to_dvs_tensor(self, img, ref):
        """
        Convert RGB PIL mixing image to same shape as DVS tensor [T, C, H, W].
        """
        T, C, H, W = ref.shape
        ...
        mix = tfm(img).float()  # [3, H, W]

        if C == 1:
            mix = mix.mean(dim=0, keepdim=True)
        elif C == 2:
            # Use two channels from RGB as pseudo polarity-like channels
            mix = mix[:2]
        else:
            mix = mix[:C]

        mix = mix.unsqueeze(0).repeat(T, 1, 1, 1)
        return mix

    def _simple_dvs_aug(self, x):
        """
        Tensor-safe augmentation for DVS frames.
        This avoids PIL-only ops from IPMix_utils.
        """
        out = x.clone()

        op = np.random.choice(["roll_t", "hflip", "noise", "cutout", "scale"])

        if op == "roll_t":
            shift = np.random.randint(0, max(1, out.shape[0]))
            ...
            sev = max(1, int(GLOBAL_ARGS.aug_severity))
            noise_std = 0.02 * sev
    ...
    def _patch_mix(self, x, mix_img_tensor):
        out = x.clone()
        T, C, H, W = out.shape

        patch_sizes = [4, 8, 16, 32]
        ps = int(np.random.choice([p for p in patch_sizes if p <= min(H, W)] or [min(H, W)]))

        y = np.random.randint(0, H - ps + 1)
        x0 = np.random.randint(0, W - ps + 1)

        op = np.random.choice(["replace", "add", "mul"])

        if op == "replace":
            out[:, :, y:y + ps, x0:x0 + ps] = mix_img_tensor[:, :, y:y + ps, x0:x0 + ps]
        elif op == "add":
            out[:, :, y:y + ps, x0:x0 + ps] = (
                out[:, :, y:y + ps, x0:x0 + ps] +
                mix_img_tensor[:, :, y:y + ps, x0:x0 + ps]
            ) / 2.0
        elif op == "mul":
            out[:, :, y:y + ps, x0:x0 + ps] = (
                out[:, :, y:y + ps, x0:x0 + ps] *
    ...
    def _ipmix_dvs(self, x, mixing_pic):
        x = self._to_tensor(x)
        mixing_tensor = self._mixing_image_to_dvs_tensor(mixing_pic, x)
        ...
        for i in range(GLOBAL_ARGS.k):
            mixed = x.clone()
            mixing_way = np.random.choice(["DVS", "P"])

            if mixing_way == "P":
                for _ in range(np.random.randint(GLOBAL_ARGS.t + 1)):
                    mixed = self._patch_mix(mixed, mixing_tensor)
            else:
                for _ in range(np.random.randint(GLOBAL_ARGS.t + 1)):
                    mixed = self._simple_dvs_aug(mixed)

            mix += ws[i] * mixed

        return ((1 - m) * x + m * mix).clamp(0, 1)
    ...
    def __getitem__(self, i):
        ...
        if self.no_jsd:
            return self._ipmix_dvs(x, mixing_pic), y
        else:
            return (
                x,
                self._ipmix_dvs(x, mixing_pic),
                self._ipmix_dvs(x, mixing_pic),
            ), y
...
def main():
    ...
    # --- DDP bootstrap (works with torchrun) ---
    ...
    if args.distributed:
        ...
        _logger.info(f"DDP init: rank={args.rank}, local_rank={local_rank}, world_size={args.world_size}")
    else:
        args.world_size = 1
        args.rank = 0
        _logger.info("Training in single-process mode.")
    ...
    args.dvs_mode = False
    if args.dataset in ["cifar10-dvs-tet", "cifar10-dvs", "cifar100-dvs", "gesture"]:
        args.dvs_mode = True
    ...
    data_config = resolve_data_config(
        vars(args), model=net, verbose=args.rank == 0
    )
    output_dir = None
    if args.rank == 0:
        if args.experiment:
    ...
    # for linear prob (guard module access)
    if args.linear_prob:
        module_ref = net.module if hasattr(net, 'module') else net
        for n, p in module_ref.named_parameters():
            if "patch_embed" in n:
    ...
    if args.dataset == "cifar10-dvs-tet":
    ...
    elif ds_name in {"tiny-imagenet", "tiny_imagenet", "tinyimagenet"}:
        train_root = os.path.join(args.data_dir, "train")
        val_root = os.path.join(args.data_dir, "val")

        if not os.path.isdir(train_root):
            raise RuntimeError(f"Tiny ImageNet train folder not found: {train_root}")
        if not os.path.isdir(val_root):
            raise RuntimeError(f"Tiny ImageNet val folder not found: {val_root}")

        dataset_train = tv_datasets.ImageFolder(train_root)
        dataset_eval = tv_datasets.ImageFolder(val_root)
        ...
        if args.rank == 0:
            ...
            _logger.info(f"TinyImageNet first train classes = {dataset_train.classes[:10]}")
            _logger.info(f"TinyImageNet first val classes   = {dataset_eval.classes[:10]}")

        assert dataset_train.class_to_idx == dataset_eval.class_to_idx, (
            "Tiny ImageNet train/val class_to_idx mismatch. "
            "This can cause random validation accuracy."
        )
    ...
    is_dvs_dataset = args.dataset in ["cifar10-dvs", "cifar10-dvs-tet", "gesture", "cifar100-dvs"]

    is_tiny_imagenet = (
    "tiny-imagenet" in ds_name
    or "tiny_imagenet" in ds_name
    or ds_name == "tinyimagenet"
    )

    use_ipmix_now = args.use_ipmix and (
        is_dvs_dataset
        or "cifar10" in ds_name
        or "cifar100" in ds_name
        or is_tiny_imagenet
    )
    ...
    if use_ipmix_now:
        ...
        train_img_size = args.img_size or data_config["input_size"][-1]

        basic_train_transform = tv_transforms.Compose([
            tv_transforms.RandomResizedCrop(
                train_img_size,
                scale=tuple(args.scale),
                ratio=tuple(args.ratio),
            ),
            tv_transforms.RandomHorizontalFlip(p=args.hflip),
            tv_transforms.ColorJitter(
                brightness=args.color_jitter,
                contrast=args.color_jitter,
                saturation=args.color_jitter,
            ) if args.color_jitter and args.color_jitter > 0 else tv_transforms.Lambda(lambda x: x),
        ])

        mixing_set_transform = tv_transforms.Compose([
            tv_transforms.Resize(train_img_size + 8),
            tv_transforms.RandomCrop(train_img_size),
        ])
        ...
        if args.rank == 0:
            _logger.info(
                f"[IPMix] Enabled for training: "
                f"dataset={args.dataset}, "
                f"img_size={train_img_size}, "
                f"basic_aug=RandomResizedCrop+HorizontalFlip+ColorJitter, "
                f"mixing_set={args.mixing_set}, "
                f"mixing_set_size={len(mixing_set)}, "
                f"k={args.k}, t={args.t}, aug_severity={args.aug_severity}, "
                f"no_jsd={args.no_jsd}"
            )
        # Wrap the train dataset with original IPMixDataset
        if is_dvs_dataset:
            dataset_train = DVSIPMixDataset(dataset_train, mixing_set, no_jsd=args.no_jsd)
        else:
            dataset_train = IPMixDataset(
    dataset_train,
    mixing_set,
    preprocess,
    no_jsd=args.no_jsd,
    basic_transform=basic_train_transform,
)
    ...
    if args.dataset in dvs_utils.DVS_DATASET:
        # ---- DVS loaders with DistributedSampler ----
        train_sampler = DistributedSampler(dataset_train, num_replicas=args.world_size, rank=args.rank,
                                           shuffle=True, drop_last=False) if args.distributed else None
        eval_sampler  = DistributedSampler(dataset_eval,  num_replicas=args.world_size, rank=args.rank,
                                           shuffle=False, drop_last=False) if args.distributed else None
        ...
        # ---- TRAIN LOADER(S) ----
        if use_ipmix_now and args.pair_batches:
            # Enforce even batch for B/2 + B/2 concatenation
            assert args.batch_size % 2 == 0, "With --pair-batches, set an even -b/--batch-size."
            half_b = args.batch_size // 2

            # Disable timm mixup/cutmix + prefetch to avoid double mixing
            args.prefetcher = False
            args.mixup = 0.0
            args.cutmix = 0.0
            args.cutmix_minmax = None

            # Build preprocess consistent with your model data_config
            preprocess = tv_transforms.Compose([
                tv_transforms.ToTensor(),
                tv_transforms.Normalize(data_config["mean"], data_config["std"])
            ])

            # Mixing set (same as above; keep severity & crops you already defined)
            mixing_set_transform = tv_transforms.Compose([
            ...
            # === Two parallel datasets: CLEAN (tensor+normalize) and IPMix (single mixed view) ===
            # Use torchvision CIFAR-100 so the IPMix stream can start from PIL images.
            try:
                clean_ds = tv_datasets.CIFAR100(root=args.data_dir, train=True, transform=preprocess, download=False)
                pil_ds   = tv_datasets.CIFAR100(root=args.data_dir, train=True, transform=None,      download=False)
            except Exception:
                _logger.warning("CIFAR-100 not found under %s, attempting to download...", args.data_dir)
                clean_ds = tv_datasets.CIFAR100(root=args.data_dir, train=True, transform=preprocess, download=True)
                pil_ds   = tv_datasets.CIFAR100(root=args.data_dir, train=True, transform=None,      download=True)

            # IPMix half-batch: single mixed image per sample (no_jsd=True)
            ipmix_half_ds = IPMixDataset(pil_ds, mixing_set, preprocess, no_jsd=True)

            # Distributed samplers for each half-dataset
            train_sampler_clean = DistributedSampler(clean_ds,      num_replicas=args.world_size, rank=args.rank,
                                                     shuffle=True, drop_last=True) if args.distributed else None
            train_sampler_aug   = DistributedSampler(ipmix_half_ds, num_replicas=args.world_size, rank=args.rank,
                                                     shuffle=True, drop_last=True) if args.distributed else None

            # Two half-batch loaders -> we will zip them per step
            loader_train_clean = torch.utils.data.DataLoader(
                clean_ds, batch_size=half_b, shuffle=(train_sampler_clean is None),
                sampler=train_sampler_clean, num_workers=args.workers, pin_memory=args.pin_mem, drop_last=True
            )
            loader_train_aug = torch.utils.data.DataLoader(
                ipmix_half_ds, batch_size=half_b, shuffle=(train_sampler_aug is None),
                sampler=train_sampler_aug, num_workers=args.workers, pin_memory=args.pin_mem, drop_last=True
            )
            loader_train = (loader_train_clean, loader_train_aug)  # <- tuple passed to paired train loop
            if args.rank == 0:
                _logger.info("Paired IPMix: per step = %d clean + %d IPMix = %d total.", half_b, half_b, args.batch_size)

        elif use_ipmix_now:
            # Single-stream IPMix (tuples (clean, aug1, aug2) if --no_jsd not set)
            train_sampler = DistributedSampler(dataset_train, num_replicas=args.world_size, rank=args.rank,
                                               shuffle=True, drop_last=True) if args.distributed else None
            loader_train = torch.utils.data.DataLoader(
            ...
            # timm default for non-IPMix training
        ...
        # ---- VAL LOADER (unchanged API; timm handles distributed sampler) ----
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            # set_epoch for distributed samplers (works for tuple of loaders too)
            if args.distributed:
                if isinstance(loader_train, tuple):
                    for ld in loader_train:
                        if hasattr(ld, "sampler") and hasattr(ld.sampler, "set_epoch"):
                            ld.sampler.set_epoch(epoch)
                elif hasattr(loader_train, "sampler") and hasattr(loader_train.sampler, "set_epoch"):
                    loader_train.sampler.set_epoch(epoch)

            if use_ipmix_now and args.pair_batches:
                train_metrics = train_one_epoch_paired_ipmix(
                    epoch, net, loader_train, optimizer, train_loss_fn, args,
                ...
                train_metrics = train_one_epoch(
                    ...
                    lr_scheduler=lr_scheduler, saver=saver, output_dir=output_dir,
                    amp_autocast=amp_autocast, loss_scaler=loss_scaler, model_ema=model_ema,
                    mixup_fn=mixup_fn, dvs_aug=train_dvs_aug, dvs_trival_aug=train_dvs_trival_aug,
                )
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if not args.prefetcher or args.dataset in dvs_utils.DVS_DATASET or isinstance(input, (tuple, list)):
            ...
            # Apply DVS basic augmentation also when IPMix returns 3 views
            if dvs_aug is not None:
                if isinstance(input, (tuple, list)):
                    input = [dvs_aug(view) for view in input]
                else:
                    input = dvs_aug(input)
                ...
                if isinstance(input, (tuple, list)):
                    ...
                    for view in input:
                        ...
                        for i in range(view.shape[0]):
                            output_imgs.append(dvs_trival_aug(view[i]))
                        aug_views.append(torch.stack(output_imgs))
                    input = aug_views
                else:
                    output_imgs = []
        ...
        if (
            args.use_ipmix
            and args.rank == 0
            and not getattr(args, "_ipmix_logged", False)
        ):
            if isinstance(input, (tuple, list)):
                _logger.info(
                    f"[IPMix] Applied PRE-ENCODING IPMix BEFORE model/spike encoder: "
                    f"mode=JSD_3_views, "
                    f"clean_shape={tuple(input[0].shape)}, "
                    f"ipmix_view1_shape={tuple(input[1].shape)}, "
                    f"ipmix_view2_shape={tuple(input[2].shape)}, "
                    f"dataset={args.dataset}, "
...
def train_one_epoch_paired_ipmix(
    epoch,
    model,
    loaders_pair,     # (loader_clean, loader_aug)
    optimizer,
    loss_fn,
    args,
    lr_scheduler=None,
    ...
    loader_clean, loader_aug = loaders_pair
    ...
    last_idx = min(len(loader_clean), len(loader_aug)) - 1
    num_updates = epoch * min(len(loader_clean), len(loader_aug))

    for batch_idx, ((xc, yc), (xa, ya)) in enumerate(zip(loader_clean, loader_aug)):
        last_batch = batch_idx == last_idx
        data_time_m.update(time.time() - end)

        # concat B/2 clean + B/2 IPMix -> B
        x = torch.cat([xc.float(), xa.float()], dim=0)
        y = torch.cat([yc, ya], dim=0)

        if args.amp and not isinstance(x, torch.cuda.HalfTensor):
            x = x.half()
        ...
        with amp_autocast():
            ...
            if isinstance(logits, (tuple, list)):
                logits = logits[0]
            if args.TET:
                loss = criterion.TET_loss(logits, y, loss_fn, means=args.TET_means, lamb=args.TET_lamb)
            else:
        ...
        if last_batch or batch_idx % args.log_interval == 0:
            ...
            if args.distributed:
                reduced_loss = reduce_tensor(loss.data, args.world_size)
                losses_m.update(reduced_loss.item(), x.size(0))
            if args.rank == 0:
                _logger.info(
                    "Train(IPMix 50/50): {} [{:>4d}/{} ({:>3.0f}%)]  "
                    "Loss: {loss.val:>9.6f} ({loss.avg:>6.4f})  "
                    "Time: {batch_time.val:.3f}s  LR: {lr:.3e}  "
                    "Data: {data_time.val:.3f} ({data_time.avg:.3f})".format(
                        epoch, batch_idx, last_idx,
                        100.0 * batch_idx / max(last_idx, 1),
                        loss=losses_m, batch_time=batch_time_m, lr=lr, data_time=data_time_m
                    )
                )
