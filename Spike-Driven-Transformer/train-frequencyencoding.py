...
from model import CenterPatchMinLift   # <-- add this line
import torch
...
# Added for center patch minlift augmentation on standard CIFAR-10
parser.add_argument(
    "--center-patch-minlift",
    action="store_true",
    default=False,
    help="Enable center patch minlift augmentation for standard CIFAR-10 training.",
)
parser.add_argument(
...
parser.add_argument(
    ...
    type=float,
    default=0.3,
    help="Minimum alpha for center patch minlift augmentation.",
)
parser.add_argument(
    "--center-patch-minlift-alpha-max",
    type=float,
    default=0.7,
    help="Maximum alpha for center patch minlift augmentation.",
)
parser.add_argument(
    "--center-patch-minlift-p",
    type=float,
    default=0.5,
    help="Probability of applying center patch minlift augmentation.",
)
...
def _is_tiny_imagenet_dataset(dataset_name):
    return str(dataset_name).lower() in {"tiny-imagenet", "tiny_imagenet", "tinyimagenet"}


def _apply_dataset_autofixes(args):
    """Auto-fix settings so CIFAR / DVS / Tiny-ImageNet work out of the box."""
    ...
    if dataset_name in {"torch/cifar10", "cifar10"}:
        ...
        if args.img_size is None:
            args.img_size = getattr(args, "dvs_resize", 64)

        if args.input_size is None or len(args.input_size) != 3:
            args.input_size = [2, args.img_size, args.img_size]

        args.mean = [0.0, 0.0]
        args.std = [1.0, 1.0]

        if args.time_steps == 4:
            args.time_steps = 16

        _logger.info(
            "Auto-configured DVS128Gesture settings: "
            f"num_classes={args.num_classes}, img_size={args.img_size}, "
            f"in_channels={args.in_channels}, time_steps={args.time_steps}"
        )
    elif _is_tiny_imagenet_dataset(dataset_name):
        if args.num_classes == 1000:
...
def main():
    ...
    if args.center_patch_minlift and args.local_rank == 0:
        _logger.info(
            "Center Patch MinLift enabled with patch_frac=%.4f, alpha_range=(%.4f, %.4f), p=%.4f"
            % (
                args.center_patch_minlift_patch_frac,
                args.center_patch_minlift_alpha_min,
                args.center_patch_minlift_alpha_max,
                args.center_patch_minlift_p,
            )
        )
    ...
    # attach post-encoding CenterPatch MinLift (training-only, no parameters)
    if args.center_patch_minlift:
        model.center_patch_min_lift = CenterPatchMinLift(
            patch_frac=args.center_patch_minlift_patch_frac,
            alpha_range=(
                args.center_patch_minlift_alpha_min,
                args.center_patch_minlift_alpha_max,
            ),
            p=args.center_patch_minlift_p,
            share_time=True,   # set False to match the old per-frame behavior
        )
    ...
    if args.dataset == "cifar10-dvs-tet":
        ...
        dvs_transform = dvs_utils.Resize(args.dvs_resize)
    ...
    elif _is_tiny_imagenet_dataset(args.dataset):
        ...
        # Support both Tiny ImageNet validation layouts:
        # 1) reorganized val/class_name/*.JPEG -> ImageFolder
        # 2) official val/images/*.JPEG + val_annotations.txt -> TinyImageNetValDataset
        if os.path.exists(os.path.join(val_root, "val_annotations.txt")) and os.path.isdir(
    ...
    if mixup_active:
        ...
        if args.prefetcher and not args.dvs_mode:
            assert (
                not num_aug_splits
    ...
    if num_aug_splits > 1 and not args.dvs_mode:
...
def center_patch_min_lift_augmentation(
    x: torch.Tensor,
    ...
    """
    CenterPatch MinLift augmentation.

    Supports:
    - standard image batches: [B, C, H, W]
    - DVS frame batches:      [B, T, C, H, W]

    For CIFAR10-DVS, the augmentation is applied independently to every
    time frame of every sample.
    """
    ...
    if original_dim == 5:
        # CIFAR10-DVS: [B, T, C, H, W] -> [B*T, C, H, W]
        B, T, C, H, W = x.shape
        x4 = x.reshape(B * T, C, H, W)
    elif original_dim == 4:
        # Standard CIFAR/ImageNet: [B, C, H, W]
        B, C, H, W = x.shape
        x4 = x
    else:
        return x
    ...
    patch = x4[:, :, y0:y1, x0:x1]

    # Per sample and per channel max inside the center patch
    M = patch.amax(dim=(2, 3), keepdim=True)
    ...
    patch_aug = (1.0 - alpha) * patch + alpha * M
    x4[:, :, y0:y1, x0:x1] = patch_aug

    return x


def is_standard_cifar10_dataset(dataset_name: str) -> bool:
    dataset_name = str(dataset_name).lower()
    return ("cifar10" in dataset_name) and ("dvs" not in dataset_name)
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        input = input.float()
        center_patch_applied = False
        if not args.prefetcher or args.dvs_mode:
