...
def _build_aug_tag(args) -> str:
    ...
    if getattr(args, "fe", False):
        ...
        # ---- PRE-encoding LayerMix ----
    if getattr(args, "use_layermix", False):
        p_str = str(getattr(args, "layermix_p", 1.0)).replace(".", "p")
        br_str = str(getattr(args, "layermix_blending_ratio", 0.5)).replace(".", "p")
        d_str = str(getattr(args, "layermix_depth", 3))
        tags.append(f"pre_layermix_p{p_str}_d{d_str}_br{br_str}")
...
def train(epoch, args):
    ...
    for batch_index, (images, labels) in enumerate(ImageNet_training_loader):
        ...
        if (
            ...
            and not getattr(args, "_layermix_logged", False)
        ):
            _log(
                f"[LayerMix] Applied PRE-ENCODING LayerMix via dataset wrapper "
                f"BEFORE model/spike encoder: "
                f"input_shape={tuple(images.shape)}, "
                f"dataset={args.dataset}, "
                f"img_size={args.img_size}, "
                f"p={args.layermix_p}, "
                f"depth={args.layermix_depth}, "
                f"blending_ratio={args.layermix_blending_ratio}, "
                f"mixing_set='{args.mixing_set}'",
                args
            )
            args._layermix_logged = True
        # PRE-encoding Center Patch MinLift
...
if __name__ == "__main__":
    ...
    type=str,
    default="imagenet",
    choices=["imagenet", "cifar10", "cifar100", "tiny-imagenet", "tiny_imagenet"],
)
    ...
    parser.add_argument("--holefill_p", type=float, default=1.0,
                        help="HoleFill apply probability.")
    ...
    parser.add_argument("--holefill_apply_in_eval", action="store_true",
                        help="Apply HoleFill in eval too.")
    
    parser.add_argument(
    "--no-basic-aug",
    action="store_true",
    help="Disable basic train augmentations such as RandomCrop/RandomHorizontalFlip."
)
    parser.add_argument(
    "--data-root",
    "--data_root",
    dest="data_root",
    type=str,
    default="/media/homes/mashalkar/data"
    )
    ...
    action="store_true",
    default=False,
    help="Enable pre-encoding LayerMix dataset augmentation.",
)
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Beta distribution parameter for LayerMix blending.",
    )
    ...
    args.dataset = str(args.dataset).lower().replace("_", "-")
    ...
    # Tiny ImageNet is small-image training, so use the CIFAR-style/small-image stem.
    args.cifar_stem = args.dataset in ["cifar10", "cifar100", "tiny-imagenet"]
    ...
    if args.dataset == "tiny-imagenet":
        ...
        if args.no_basic_aug:
            train_transform = tv_transforms.Compose([
                tv_transforms.Resize((args.img_size, args.img_size)),
                tv_transforms.ToTensor(),
                tv_transforms.Normalize(mean, std),
            ])
        else:
            train_transform = tv_transforms.Compose([
                tv_transforms.RandomCrop(args.img_size, padding=4),
                tv_transforms.RandomHorizontalFlip(),
                tv_transforms.ToTensor(),
                tv_transforms.Normalize(mean, std),
            ])

        train_dataset = tv_datasets.ImageFolder(args.traindir, transform=train_transform)
        ...
        if is_rank0:
            _log(
                f"[DATA] Tiny ImageNet train dataset created directly like PatchMix: "
                f"root={args.traindir}, size={len(train_dataset)}, "
                f"classes={len(train_dataset.classes)}, img_size={args.img_size}, "
                f"basic_aug={not args.no_basic_aug}",
                args
            )
        ...
        ImageNet_training_loader, train_sampler = get_training_dataloader(
            ...
            shuffle=False,
            sampler=1,
            no_basic_aug=args.no_basic_aug,
        )

        # =========================
    # Optional PRE-encoding LayerMix dataset wrapper
    # =========================
    if args.use_layermix:
        if args.dataset not in ["cifar10", "cifar100", "tiny-imagenet"]:
            raise ValueError("LayerMix is currently supported for cifar10/cifar100/tiny-imagenet in this MS-ResNet setup.")

        if args.mixing_set is None or len(str(args.mixing_set)) == 0:
            raise ValueError("--mixing-set is required when using --use-layermix")

        if not os.path.isdir(args.mixing_set):
            raise ValueError(f"LayerMix mixing set path does not exist: {args.mixing_set}")

        old_train_dataset = ImageNet_training_loader.dataset

        layermix_train_dataset = PreEncodingLayerMixDataset(
        dataset=old_train_dataset,
        mixing_set_root=args.mixing_set,
        img_size=int(args.img_size),   # 64 for Tiny ImageNet
        depth=int(args.layermix_depth),
        blending_ratio=float(args.layermix_blending_ratio),
        p=float(args.layermix_p),
    )

        ImageNet_training_loader = torch.utils.data.DataLoader(
            layermix_train_dataset,
            batch_size=args.b // max(num_gpus, 1),
            shuffle=False,
            sampler=train_sampler,
            num_workers=2,
        ...
        if is_rank0:
            _log(
                f"[AUG] PreEncodingLayerMixDataset enabled: "
                f"{{'p': {args.layermix_p}, "
                f"'depth': {args.layermix_depth}, "
                f"'blending_ratio': {args.layermix_blending_ratio}, "
                f"'mixing_set': '{args.mixing_set}', "
                f"'mixing_set_size': {len(layermix_train_dataset.mixing_set)}}}",
                args
            )
    ...
    if args.dataset == "tiny-imagenet":
        ...
        val_dataset = tv_datasets.ImageFolder(args.valdir, transform=val_transform)
        ...
        if is_rank0:
            _log(
                f"[DATA] Tiny ImageNet val dataset created directly like PatchMix: "
                f"root={args.valdir}, size={len(val_dataset)}, "
                f"classes={len(val_dataset.classes)}, img_size={args.img_size}",
                args
            )
