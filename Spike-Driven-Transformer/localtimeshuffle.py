...
class LocalTimeShuffle(torch.nn.Module):
    """
    Local TimeShuffle (windowed shuffle):
    Split time axis into windows of size W, shuffle indices within each window.
    """
    ...
    @staticmethod
    def _infer_time_dim(x: torch.Tensor, time_steps: int | None):
        """
        Infer the time dimension.

        For SpikeFormer post-encoding features, the expected shape is:
            (T, B, C, H, W)

        Therefore, prefer dim=0 first.
        This avoids accidentally shuffling the batch dimension when B == T.
        """
        if x.dim() == 5:
            if time_steps is not None and x.shape[0] == time_steps:
                return 0  # SpikeFormer post-encoding: (T, B, C, H, W)

            if time_steps is not None and x.shape[1] == time_steps:
                return 1  # fallback for possible (B, T, C, H, W)

            return 0  # safest default for your SpikeFormer case

        if time_steps is None:
            return None

        cand = [i for i, s in enumerate(x.shape) if s == time_steps]
        if not cand:
            return None
    ...
    def _make_index(self, T: int, device):
        ws = min(max(self.window_size, 1), T)
        if ws <= 1:
    ...
    def forward(self, x: torch.Tensor):
        if self.window_size <= 1 or x.dim() < 2:
            return x

        time_dim = self.time_dim if self.time_dim is not None else self._infer_time_dim(x, self.time_steps)
        if time_dim is None:
            # no detectable time axis -> do nothing
            return x
        ...
        # If per_sample is True, we apply decision per sample only when batch is dim 0 and time_dim != 0.
        if self.per_sample and time_dim != 0:
            B = x.size(0)
            out = []
            for b in range(B):
                xb = x[b]  # remove batch dim
                if torch.rand((), device=x.device) < self.p:
                    idx = self._make_index(T, x.device)
                    if idx is not None:
                        # after removing batch dim, time_dim shifts by -1
                        xb = xb.index_select(time_dim - 1, idx)
                out.append(xb)
            return torch.stack(out, dim=0)
        else:
            # single decision for whole tensor
            if torch.rand((), device=x.device) >= self.p:
                return x
            idx = self._make_index(T, x.device)
            if idx is None:
...
def _auto_configure_for_dataset(args):
    ...
    if ds in ["torch/cifar10", "cifar10"]:
        ...
        if args.val_split == "validation":
            args.val_split = "test"
        if args.train_split in ["validation", "val", "test"]:
    ...
    elif ds in ["tiny-imagenet", "tiny_imagenet", "tinyimagenet"]:
        if args.num_classes is None or args.num_classes == 1000:
        ...
        # CIFAR10-DVS has 2 event channels
        args.in_channels = 2

        # Your CIFAR10-DVS pipeline resizes frames to 64x64
        if args.img_size is None or args.img_size == 224:
            args.img_size = 64

        # Important for timm resolve_data_config:
        # mean/std length must match in_channels
        if args.input_size is None or len(args.input_size) != 3 or args.input_size[0] != 2:
...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="Windowed shuffle along the time axis (for inputs that contain a time dimension).",
)
parser.add_argument(
    "--lts-window",
    type=int,
    default=2,
    help="Window size (in frames) for local time shuffle. Example: 2, 4, 8 ...",
)
parser.add_argument(
    "--lts-prob",
    type=float,
    default=1.0,
    help="Probability to apply the local time shuffle (0..1).",
)
parser.add_argument(
    "--lts-per-sample",
    action="store_true",
    default=False,
    help="Use different shuffle per sample (only possible when batch dim is first).",
)
...
def _parse_args():
    ...
    # Auto-fix common dataset-specific settings
    args = _auto_configure_for_dataset(args)
...
def main():
    ...
    if args.dataset in ["cifar10-dvs-tet", "cifar10-dvs", "gesture"]:
        args.dvs_mode = True
    args.is_dvs_dataset = args.dataset in dvs_utils.DVS_DATASET or args.dataset == "gesture"
    model = create_model(
        args.model,
        T=args.time_steps,
    ...
    if args.local_time_shuffle:
        model.local_time_shuffle = LocalTimeShuffle(
            window_size=args.lts_window,
            p=args.lts_prob,
            time_steps=args.time_steps,
            per_sample=args.lts_per_sample,
        )

        if args.local_rank == 0:
            _logger.info(
                f"[LocalTimeShuffle] Attached to model for POST-ENCODING use: "
                f"window={args.lts_window}, prob={args.lts_prob}, "
                f"time_steps={args.time_steps}, per_sample={args.lts_per_sample}"
            )
    else:
        model.local_time_shuffle = None
    if args.local_rank == 0:
    ...
    time_shuffle_aug = None
    if args.local_time_shuffle:
        time_shuffle_aug = LocalTimeShuffle(
            window_size=args.lts_window,
            p=args.lts_prob,
            time_steps=args.time_steps,
            per_sample=args.lts_per_sample,
            time_dim=1,   # DVSGesture input shape: [B, T, C, H, W]
        )
        if args.local_rank == 0:
            _logger.info(
                f"[LocalTimeShuffle] INPUT-level enabled: "
                f"window={args.lts_window}, prob={args.lts_prob}, "
                f"time_steps={args.time_steps}, per_sample={args.lts_per_sample}"
            )
    ...
    if num_aug_splits > 1 and not args.is_dvs_dataset:
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            train_metrics = train_one_epoch(
                ...
                time_shuffle_aug=time_shuffle_aug,
            )
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        input = input.float()
        if not args.prefetcher or args.is_dvs_dataset:
            ...
            if dvs_trival_aug is not None:
                ...
                del output
            if time_shuffle_aug is not None:
                input = time_shuffle_aug(input)
...
def validate(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix=""):
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            target_upper_bound = args.num_classes if args.num_classes is not None else 1000
            if (target >= target_upper_bound).sum() != 0 or (target < 0).sum() != 0:
                print(target)
