...
# === PatchDropout additions ===
class PatchDropoutAug:
    """
    Training-time PatchDropout augmentation (image-space masking of entire patches).
    - Randomly keeps a subset of non-overlapping P x P patches (uniform sampling without replacement).
    - Keep rate can be fixed or sampled uniformly in [min_keep, 1] per call (augmentation variant from the paper).
    - Inference should use all patches (so do NOT apply this in validation/test).

    Notes (paper methodology):
    * Paper drops tokens after patch embedding & adds positional embeddings prior to sampling; CLS retained.
    * Here we deny the same information pattern at the input by masking full patches,
      while preserving the training protocol (random keep-rate per iteration, full input at test).
    """
    def __init__(self, patch_size: int = 16, keep: float = 1.0, min_keep: float | None = None):
    ...
    def _sample_keep(self, device):
        if self.min_keep is not None:
            # Uniform keep-rate as augmentation, e.g., U[0.5, 1.0] (paper Table 7)
    ...
    def __call__(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if x.dim() == 4:
            ...
            if not self._warned_shape:
                print("[PatchDropout] Unsupported input dim; skipping.", flush=True)
        ...
        if (H < P) or (W < P) or (H % P != 0) or (W % P != 0):
            if not self._warned_shape:
                print(f"[PatchDropout] HxW={H}x{W} not divisible by patch_size={P}; skipping dropout for this batch.", flush=True)
                self._warned_shape = True
            return x
        ...
        n_w = W // P
        N = n_h * n_w
        k = max(1, int(round(keep_rate * N)))

        # Build a per-sample spatial mask over the patch grid, then expand to pixels
        masks = []
        for _ in range(B):
            idx = torch.randperm(N, device=device)[:k]
            m = torch.zeros(N, device=device, dtype=torch.bool)
            m[idx] = True
            m = m.view(n_h, n_w)  # (n_h, n_w)
            # Expand each True entry to a P x P block
            m = m.unsqueeze(0).unsqueeze(0)  # (1,1,n_h,n_w)
            m = m.repeat_interleave(P, dim=2).repeat_interleave(P, dim=3)  # (1,1,H,W)
            masks.append(m)
        ...
        if T is None:
            ...
            # Same mask for all time steps to preserve temporal coherence: (B,T,C,H,W)
            mask_t = mask.unsqueeze(1)  # (B,1,1,H,W)
            x = x * mask_t

        return x
# === End PatchDropout additions ===
...
# Model EMA
parser.add_argument("--model-ema", action="store_true", default=False, help="Enable moving average of model weights")
parser.add_argument(
    "--model-ema-force-cpu",
    action="store_true",
    default=False,
...
parser.add_argument("--model-ema-decay", type=float, default=0.9998, help="EMA decay (default: 0.9998)")
...
parser.add_argument("--checkpoint-hist", type=int, default=10, metavar="N", help="number of checkpoints to keep")
...
parser.add_argument("--amp", action="store_true", default=False, help="use APEX AMP or native AMP")
...
parser.add_argument("--experiment", default="", type=str, metavar="NAME", help="name of train experiment")
parser.add_argument(
    "--eval-metric",
    default="top1",
    type=str,
...
parser.add_argument("--tta", type=int, default=0, metavar="N", help="Test-time augmentation oversampling factor")
...
# === PatchDropout CLI flags (additions) ===
parser.add_argument("--patchdropout", action="store_true", default=False,
                    help="Enable PatchDropout augmentation during TRAINING only.")
parser.add_argument("--pd-keep", type=float, default=1.0,
                    help="Fixed keep rate (0<keep<=1). Use <1 to drop patches.")
parser.add_argument("--pd-min-keep", type=float, default=None,
                    help="If set, sample keep ~ Uniform(pd_min_keep, 1.0) each step (paper's augmentation).")
parser.add_argument("--pd-patch-size", type=int, default=None,
                    help="Patch size for dropout masking; defaults to --patch-size or 16.")
# === End additions ===
...
def main():
    ...
    if args.local_rank == 0:
        ...
        _logger.info(
            str(torchinfo.summary(model, (2, args.in_channels, args.img_size, args.img_size)))
        )
    ...
    # === PatchDropout instantiation (additions) ===
    patchdropout_aug = None
    if args.patchdropout:
        pd_patch = args.pd_patch_size if args.pd_patch_size is not None else (args.patch_size if args.patch_size else 16)
        patchdropout_aug = PatchDropoutAug(patch_size=pd_patch, keep=args.pd_keep, min_keep=args.pd_min_keep)
        if args.local_rank == 0:
            if args.pd_min_keep is not None:
                _logger.info(f"[PatchDropout] Enabled with keep ~ U[{args.pd_min_keep}, 1.0], P={pd_patch}")
            else:
                _logger.info(f"[PatchDropout] Enabled with fixed keep={args.pd_keep}, P={pd_patch}")
    # === End additions ===
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            train_metrics = train_one_epoch(
                ...
                patchdropout_aug=patchdropout_aug,  # <-- use in training only
            )
...
def train_one_epoch(
    ...
    dvs_aug=None,
    dvs_trival_aug=None,
    patchdropout_aug=None,  # <-- addition
):
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if not args.prefetcher or args.dataset in dvs_utils.DVS_DATASET:
            ...
            # === PatchDropout application (training only) ===
            if patchdropout_aug is not None:
                input = patchdropout_aug(input)
            # === End PatchDropout ===
