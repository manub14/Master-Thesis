...
# move model to the chosen device; channels_last if requested
    ...
    if args.resume:
        ...
        if args.local_rank == 0:
            _logger.info("Loaded checkpoint from %s", args.resume)
    ...
    elif args.dataset in ["tiny-imagenet", "tiny_imagenet"]:
        if args.val_split.lower() == "test":
            raise ValueError(
                "Tiny-ImageNet test split has no labels, so loss/top1/top5 cannot be computed. "
                "Use --val-split val for evaluation."
            )

        split_name = "val" if args.val_split.lower() == "validation" else args.val_split
        eval_root = os.path.join(args.data_dir, split_name)
        if not os.path.isdir(eval_root):
            raise FileNotFoundError(f"Missing Tiny-ImageNet split directory: {eval_root}")

        tiny_eval_tf = transforms.Compose([
            transforms.Resize(args.img_size if args.img_size is not None else 64),
            transforms.CenterCrop(args.img_size if args.img_size is not None else 64),
            transforms.ToTensor(),
            transforms.Normalize(mean=data_config["mean"], std=data_config["std"]),
        ])

        dataset_eval = ImageFolder(eval_root, transform=tiny_eval_tf)
