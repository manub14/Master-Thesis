...
def patchmix_batch(x, p: float = 0.5, alpha: float = 0.2, beta: float = 0.2, grid: int = 2):
    """
    x: Tensor with shape (B, C, H, W) or (B, T, C, H, W).
    Applies PatchMix independently per sample, sharing shuffle/? across all channels/time dims.
    """
    if not (x.ndim in (4, 5)):
        return x  # unsupported shape, skip gracefully
    ...
    if x.ndim == 4:
        ...
        for b in range(B):
            T, C, H, W = x[b].shape
            xb = x[b].reshape(T * C, H, W)  # collapse (T,C) to apply consistent patch ops
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if not args.prefetcher or args.dataset in dvs_utils.DVS_DATASET:
            ...
            # ---- Apply PatchMix BEFORE MixUp/CutMix ----
            if args.patchmix:
                try:
                    ...
                    # If grid doesn't divide H/W, skip gracefully for this batch and warn once
