...
class TinyImageNetValDataset(Dataset):
    """
    Tiny ImageNet validation dataset for official layout:
      val/images/*.JPEG
...
parser.add_argument(
    ...
    type=int,
    default=64,
    help="Resize size for DVS frames",
)
parser.add_argument(
    "--pretrained",
    action="store_true",
...
parser.add_argument(
    "--postenc-holefill-mode",
    type=str,
    default=None,
    help='Post-encoding hole-fill mode: "spatial", "spatiotemporal" or None',
)
parser.add_argument(
    "--postenc-holefill-prob",
    type=float,
    default=1.0,
    help="Probability to apply post-encoding hole-fill augmentation during training ",
)
...
class PostEncodingHoleFill(nn.Module):
    """
    Post-encoding hole-fill augmentation.

    Supports:
      - spatial: fill a zero if all 8 spatial neighbors are 1
      - spatiotemporal: fill a zero if all 26 3D neighbors in (t, h, w) are 1

    Expected internal working layout: B, T, C, H, W
    Also supports:
      - T, B, C, H, W
      - B, C, H, W
    """

    def __init__(self, mode="spatial", prob=1.0, time_steps=None):
        super().__init__()
        if mode is not None:
            mode = mode.lower()
        if mode not in (None, "spatial", "spatiotemporal"):
            raise ValueError(
                f"Unsupported post-encoding hole-fill mode: {mode}. "
                f"Expected one of: None, 'spatial', 'spatiotemporal'."
            )
        self.mode = mode
        self.prob = prob
    ...
    def _to_btchw(self, x):
        ...
        # Prefer explicit time_steps if available
        ...
        # Heuristic fallback
        # Common cases:
        #   TBCHW where T is small and B is larger
        #   BTCHW otherwise
    ...
    def _spatial_fill(self, x_btchw):
        ...
        filled = torch.where(fill_mask, torch.ones_like(x_orig_2d), x_orig_2d)
        return filled.reshape(B, T, C, H, W)
    ...
    def _spatiotemporal_fill(self, x_btchw):
        ...
        x_bin_3d = x_bin.permute(0, 2, 1, 3, 4).contiguous().reshape(B * C, 1, T, H, W)
        x_orig_3d = x_btchw.permute(0, 2, 1, 3, 4).contiguous().reshape(B * C, 1, T, H, W)
        ...
        filled = torch.where(fill_mask, torch.ones_like(x_orig_3d), x_orig_3d)
        filled = filled.reshape(B, C, T, H, W).permute(0, 2, 1, 3, 4).contiguous()
        return filled
    ...
    def forward(self, x):
        ...
        x_btchw, layout = self._to_btchw(x)
        if layout is None:
            return x
        ...
        if self.mode == "spatial":
            x_filled = self._spatial_fill(x_btchw)
        elif self.mode == "spatiotemporal":
            x_filled = self._spatiotemporal_fill(x_btchw)
        else:
            return x

        if self.prob < 1.0:
            apply_mask = (
                torch.rand(B, device=x_btchw.device) < self.prob
            ).view(B, 1, 1, 1, 1)
            x_btchw = torch.where(apply_mask, x_filled, x_btchw)
        else:
            x_btchw = x_filled

        return self._from_btchw(x_btchw, layout)


def _apply_holefill_to_output(output, holefill_aug):
    if torch.is_tensor(output):
        return holefill_aug(output)
    elif isinstance(output, tuple):
        return tuple(_apply_holefill_to_output(o, holefill_aug) for o in output)
    elif isinstance(output, list):
        return [_apply_holefill_to_output(o, holefill_aug) for o in output]
    elif isinstance(output, dict):
        return {k: _apply_holefill_to_output(v, holefill_aug) for k, v in output.items()}
    else:
        return output


def register_postenc_holefill_hook(model, args):
    """
    Fallback registration:
    if the model constructor does not natively support postenc_holefill_* args,
    we apply the augmentation as a forward hook on the earliest likely post-encoding module.
    """
    if args.postenc_holefill_mode is None:
        return False

    holefill_aug = PostEncodingHoleFill(
        mode=args.postenc_holefill_mode,
        prob=args.postenc_holefill_prob,
    ...
    for name in candidate_names:
        if hasattr(model, name):
            module_candidate = getattr(model, name)
            if isinstance(module_candidate, nn.Module):
                target_module = module_candidate
                target_name = name
                break
    ...
    if target_module is None:
        for name, module_candidate in model.named_modules():
            lname = name.lower()
            if any(key in lname for key in ["patch_embed", "sps", "stem", "tokenizer", "embedding"]):
                target_module = module_candidate
                target_name = name
                break
    ...
    if target_module is None:
        _logger.warning(
            "Could not find a suitable module for post-encoding hole-fill hook. "
            "No external hook was registered."
        )
        return False

    def _holefill_hook(module, inputs, output):
        return _apply_holefill_to_output(output, holefill_aug)

    handle = target_module.register_forward_hook(_holefill_hook)
    model._postenc_holefill_handle = handle
    model._postenc_holefill_module = holefill_aug

    _logger.info(
        f"Registered post-encoding hole-fill hook on module '{target_name}' "
        f"with mode='{args.postenc_holefill_mode}', prob={args.postenc_holefill_prob}."
    )
    return True
...
CIFAR10_DVS_NAMES = {"cifar10-dvs", "cifar10-dvs-tet"}
GESTURE_NAMES = {"gesture", "dvs128gesture", "dvs128-gesture", "dvsgesture128"}


def _is_tiny_imagenet(dataset_name):
    return str(dataset_name).lower() in TINYIMAGENET_NAMES


def _apply_dataset_autofixes(args):
    dataset_name = str(args.dataset).lower()

    if dataset_name in TINYIMAGENET_NAMES:
        ...
        _logger.info(
            ...
            "num_classes=200, img_size=64, in_channels=3, val_split=val"
        )

    elif dataset_name in CIFAR10_DVS_NAMES:
        if args.num_classes == 1000:
            args.num_classes = 10
        if args.img_size is None or args.img_size == 224:
        ...
        _logger.info(
            "Auto-configured CIFAR10-DVS settings: "
            f"num_classes={args.num_classes}, img_size={args.img_size}, "
            f"in_channels={args.in_channels}, dvs_resize={args.dvs_resize}"
        )

    elif dataset_name in GESTURE_NAMES:
        if args.num_classes == 1000:
            args.num_classes = 11
        if args.img_size is None or args.img_size == 224:
            args.img_size = args.dvs_resize
        ...
        _logger.info(
            "Auto-configured DVSGesture128 settings: "
            f"num_classes={args.num_classes}, img_size={args.img_size}, "
            f"in_channels={args.in_channels}, dvs_resize={args.dvs_resize}"
        )
...
def main():
    ...
    # safer default for single-GPU runs
    args.device = "cuda:0"
    args.world_size = 1
    ...
    args.dvs_mode = False
    if str(args.dataset).lower() in CIFAR10_DVS_NAMES or str(args.dataset).lower() in GESTURE_NAMES:
        args.dvs_mode = True
    ...
    if args.postenc_holefill_mode is not None:
        hook_ok = register_postenc_holefill_hook(model, args)
        if not hook_ok:
            raise RuntimeError(
                "postenc-holefill-mode was enabled, but no suitable post-encoding "
                "module was found for the hook. Check the module names in model.py."
            )
    ...
    if args.local_rank == 0:
        ...
        _logger.info(str(torchinfo.summary(model, summary_input)))
    ...
    if args.dataset == "cifar10-dvs-tet":
    ...
    elif _is_tiny_imagenet(args.dataset):
        ...
        official_val = (
            os.path.isdir(os.path.join(val_root, "images"))
            and os.path.isfile(os.path.join(val_root, "val_annotations.txt"))
        )

        if official_val:
            dataset_eval = TinyImageNetValDataset(val_root)
            _logger.info("Using TinyImageNetValDataset for official val/images layout.")
        else:
            dataset_eval = ImageFolder(val_root)
            _logger.info("Using ImageFolder for already-converted Tiny ImageNet val layout.")
    ...
    if args.local_rank == 0:
        ...
        if args.postenc_holefill_mode is not None:
            _logger.info(
                f"Post-encoding hole-fill is enabled for training: "
                f"mode={args.postenc_holefill_mode}, prob={args.postenc_holefill_prob}"
            )
