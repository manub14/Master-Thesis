...
parser.add_argument('--double-batch', action='store_true', default=False,
                    help='Train on clean+IPMix per sample (2 times effective batch).')
...
class IPMixDataset(torch.utils.data.Dataset):
    """
    Dataset wrapper to perform IPMix like the original, with selectable return mode:
      - mode='single' : returns only one IPMix view (x_ipmix, y)
      - mode='pair'   : returns (x_clean, x_ipmix), y          <-- for double-batch
      - mode='triple' : returns (x_clean, x_ipmix1, x_ipmix2), y  (for JSD regularization)
    """
    def __init__(self, dataset, mixing_set, preprocess, mode='triple'):
        assert mode in {'single', 'pair', 'triple'}
    ...
    def __getitem__(self, i):
        x, y = self.dataset[i]                       # PIL image, label
        rnd_idx = np.random.choice(len(self.mixing_set))
        mixing_pic, _ = self.mixing_set[rnd_idx]     # PIL image

        if self.mode == 'single':
            return ipmix(x, mixing_pic, self.preprocess), y

        clean = self.preprocess(x)
        aug1  = ipmix(x, mixing_pic, self.preprocess)

        if self.mode == 'pair':
            return (clean, aug1), y

        # mode == 'triple'
        aug2 = ipmix(x, mixing_pic, self.preprocess)
        return (clean, aug1, aug2), y
...
def main():
    ...
    if use_ipmix_now:
        ...
        # Decide dataset return mode
        if args.double_batch:
            ipmix_mode = 'pair'          # clean + ONE ipmix view ? 2x effective batch
        else:
            ipmix_mode = 'single' if args.no_jsd else 'triple'

        # Wrap the train dataset
        dataset_train = IPMixDataset(dataset_train, mixing_set, preprocess, mode=ipmix_mode)
...
def train_one_epoch(
    ...
    for batch_idx, (inp, target) in enumerate(loader):
        ...
        if isinstance(inp, (tuple, list)):
            inp = [im.float() for im in inp]
        else:
            inp = inp.float()

        # move to device
        if not args.prefetcher or args.dataset in dvs_utils.DVS_DATASET or isinstance(inp, (tuple, list)):
            if isinstance(inp, (tuple, list)):
                if args.amp:
                    inp = [im.half() if not isinstance(im, torch.cuda.HalfTensor) else im for im in inp]
            ...
            if dvs_aug is not None and not isinstance(inp, (tuple, list)):
                inp = dvs_aug(inp)
            if dvs_trival_aug is not None and not isinstance(inp, (tuple, list)):
                out_imgs = [dvs_trival_aug(inp[i]) for i in range(inp.shape[0])]
                inp = torch.stack(out_imgs)
                del out_imgs

            if mixup_fn is not None and not isinstance(inp, (tuple, list)):
                inp, target = mixup_fn(inp, target)

        if args.channels_last:
            if isinstance(inp, (tuple, list)):
                inp = [im.contiguous(memory_format=torch.channels_last) for im in inp]
            else:
                inp = inp.contiguous(memory_format=torch.channels_last)

        with amp_autocast():
            if isinstance(inp, (tuple, list)):
                # --- tuple inputs from IPMixDataset ---
                if len(inp) == 2:
                    # CLEAN + ONE AUG  ? 2x effective batch   (double-batch path)
                    clean, aug = inp
                    images_all = torch.cat([clean, aug], dim=0)          # [2B, C, H, W]
                    logits_all = model(images_all)
                    if isinstance(logits_all, (tuple, list)):
                        logits_all = logits_all[0]
                    b = clean.size(0)
                    logits_clean, logits_aug = torch.split(logits_all, b, dim=0)

                    # CE on BOTH halves (equal weight)
                    loss = 0.5 * (F.cross_entropy(logits_clean, target) +
                                  F.cross_entropy(logits_aug,   target))

                    effective_batch = images_all.shape[0]

                elif len(inp) == 3:
                    # CLEAN + AUG1 + AUG2  ? original JSD recipe
                    images_all = torch.cat(inp, dim=0)                    # [3B, C, H, W]
                    logits_all = model(images_all)
                    if isinstance(logits_all, (tuple, list)):
                        logits_all = logits_all[0]
                    ...
                    if not args.no_jsd:
                        ...
                        jsd = (F.kl_div(p_mix, p_clean, reduction='batchmean') +
                               F.kl_div(p_mix, p_aug1,  reduction='batchmean') +
                               F.kl_div(p_mix, p_aug2,  reduction='batchmean')) / 3.0
                        loss = loss + 12.0 * jsd

                    effective_batch = images_all.shape[0]

                else:
                    raise RuntimeError(f"Unexpected tuple length from dataset: {len(inp)}")
                ...
                effective_batch = inp.shape[0]

        sample_number += effective_batch
        if not args.distributed:
            losses_m.update(loss.item(), effective_batch)
        ...
        if last_batch or batch_idx % args.log_interval == 0:
            ...
            if args.distributed:
                ...
                losses_m.update(reduced_loss.item(), effective_batch)

            if args.local_rank == 0:
                rate_now = effective_batch * args.world_size / max(batch_time_m.val, 1e-8)
                rate_avg = effective_batch * args.world_size / max(batch_time_m.avg, 1e-8)
                _logger.info(
                    "Train: {} [{:>4d}/{} ({:>3.0f}%)]  "
                    ...
                    "Data: {data_time.val:.3f} ({data_time.avg:.3f})".format(
                        ...
                        rate=rate_now, rate_avg=rate_avg, lr=lr, data_time=data_time_m,
                    )
                )

                # keep your save_images block as-is (it skips tuple inputs)
