...
from module.classmix_postenc import ClassMixPostEncoder
try:
...
parser.add_argument(
    ...
    type=int,
    default=64,
    help="Resize DVS frame input to this spatial size.",
)
parser.add_argument(
    "--dvs-aug",
    action="store_true",
...
parser.add_argument(
    ...
    type=float,
    default=0.5,
    help="Probability to apply post-encoding ClassMix augmentation (0 disables).",
)
...
def _is_classmix_enabled(args, model):
    base_model = _unwrap_model(model)
    return (
        getattr(args, "classmix_p", 0.0) > 0
        and hasattr(base_model, "postenc_aug")
        and base_model.postenc_aug is not None
    )

def _set_postenc_targets(model, target):
    base_model = _unwrap_model(model)
    base_model._postenc_targets = target


def _clear_postenc_targets(model):
    base_model = _unwrap_model(model)
    if hasattr(base_model, "_postenc_targets"):
        delattr(base_model, "_postenc_targets")


def _sync_postenc_aug_mode(model, train_mode):
    base_model = _unwrap_model(model)
    if hasattr(base_model, "postenc_aug") and base_model.postenc_aug is not None:
        if train_mode:
            base_model.postenc_aug.train()
        else:
            base_model.postenc_aug.eval()
...
def main():
    ...
    args.dvs_mode = args.dataset in ["cifar10-dvs-tet", "cifar10-dvs", "gesture"]
    ...
    if getattr(args, "classmix_p", 0.0) > 0:
        model.postenc_aug = ClassMixPostEncoder(p=args.classmix_p)
        if args.local_rank == 0:
            _logger.info(
                f"Enabled post-encoding ClassMix/ClassBatchMix for {args.dataset} with p={args.classmix_p}."
            )
    else:
        model.postenc_aug = None
    ...
    _sync_postenc_aug_mode(model, train_mode=True)
    ...
    if args.dataset == "cifar10-dvs-tet":
        ...
        gesture_transform = dvs_utils.Resize(args.dvs_resize)
    ...
    if getattr(args, "classmix_p", 0.0) > 0 and mixup_active:
        raise ValueError("Do not enable ClassMix/ClassBatchMix together with mixup/cutmix.")
    if mixup_active:
        mixup_args = dict(
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        classmix_active = _is_classmix_enabled(args, model)
        
        if args.local_rank == 0 and batch_idx % args.log_interval == 0:
            if classmix_active:
                unique_classes = target.unique().numel() if target.ndim == 1 else "soft-target"
                _logger.info(
                    f"[ClassBatchMix] ACTIVE batch={batch_idx}, "
                ...
                _logger.info(
                    f"[ClassBatchMix] OFF batch={batch_idx}, classmix_p={args.classmix_p}"
                )


        if classmix_active:
            _set_postenc_targets(model, target)
...
def validate(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix=""):
    ...
    model.eval()
    _sync_postenc_aug_mode(model, train_mode=False)
    _clear_postenc_targets(model)
    # functional.reset_net(model)
