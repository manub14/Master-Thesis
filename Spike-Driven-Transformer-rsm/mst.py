...
def Masking_Spike(x, masking_ratio=0.):
    mask = torch.rand(size=(x.shape), device=x.device).gt(masking_ratio)
    return x * mask
...
class SpikingWindowAttention(nn.Module):
    r"""Window based  spiking  multi-head self attention (W-MSA) module with relative position bias.
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.,
        ...
        self.masking_ratio = masking_ratio  # masking rate
    ...
    def forward(self, x, mask=None):
        ...
        q_spk = self.q_if(q)
        k_spk = self.k_if(k)
        q_spk_masked = Masking_Spike(q_spk, masking_ratio=self.masking_ratio)
        k_spk_masked = Masking_Spike(k_spk, masking_ratio=self.masking_ratio)

        attn = ((q_spk_masked @ k_spk_masked.transpose(-2, -1)) * self.scale)
        attn_spk = self.attn_if(attn)
        attn_spk_masked = Masking_Spike(attn_spk, masking_ratio=self.masking_ratio)
        ...
        attn = attn_spk_masked + relative_position_bias.unsqueeze(0)
        if mask is not None:
            ...
            attn = self.softmax(attn)
        x = (attn @ v_spk).transpose(1, 2).reshape(B_, N, C)
        x = self.attn_v_if(x)
        x = self.proj(x)
        x = self.proj_if(x)
        x = self.proj_drop(x)
...
class MaskedSpikingTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        self.shortcut1_if = nn.ReLU()
        self.attn = SpikingWindowAttention(
            dim, window_size=to_2tuple(self.window_size), num_heads=num_heads,
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, masking_ratio=self.masking_ratio)
...
class SpikingPatchMerging(nn.Module):
    ...
    def __init__(self, input_resolution, dim, masking_ratio=0.):
        ...
        self.norm = nn.BatchNorm1d(4 * dim)
        self.reduction_if = nn.ReLU()
        self.masking_ratio = masking_ratio
...
class BasicLayer(nn.Module):
    """ A basic Masked Spiking Transformer layer for one stage.
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        self.blocks = nn.ModuleList([
            MaskedSpikingTransformerBlock(dim=dim, input_resolution=input_resolution,
        ...
        if downsample is not None:
            self.downsample = downsample(input_resolution, dim=dim, masking_ratio=self.masking_ratio)
        else:
...
class SpikingPatchEmbed(nn.Module):
    r""" Image to Spiking Patch Embedding
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None, masking_ratio=0.):
        super().__init__()
...
class MaskedSpikingTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        self.patch_embed = SpikingPatchEmbed(
            ...
            norm_layer=norm_layer if self.patch_norm else None, masking_ratio=self.masking_ratio)
