...
parser.add_argument(
    ...
    type=int,
    default=64,
    help="Resize DVS frames to this spatial size. Use 0 to disable resize.",
)
...
parser.add_argument(
    ...
    default=0.0,
    metavar="PCT",
    help="Random Spike Masking ratio (0.0 disables)",
)
parser.add_argument(
    "--ratio",
    type=float,
...
def _apply_dataset_autofixes(args):
    ...
    if dataset_name in {"torch/cifar10", "cifar10"}:
        ...
        _logger.info(
            ...
            "num_classes=200, img_size=64, in_channels=3, train_split=train, val_split=val"
        )
...
class TinyImageNetValDataset(torch.utils.data.Dataset):
    """
    For official Tiny ImageNet validation layout:
      val/images/*.JPEG
...
def main():
    ...
    if args.dataset == "cifar10-dvs-tet":
        ...
        dvs_transform = None
        if args.dvs_resize and args.dvs_resize > 0:
        ...
        # Case 1: validation already reorganized as val/class_name/images
        val_has_class_folders = any(
            os.path.isdir(os.path.join(val_root, x))
            for x in os.listdir(val_root)
            if x != "images"
        )

        if val_has_class_folders:
            dataset_eval = ImageFolder(val_root)
            _logger.info("Using Tiny ImageNet validation as ImageFolder.")
        else:
            dataset_eval = TinyImageNetValDataset(val_root)
            _logger.info("Using official Tiny ImageNet val_annotations.txt format.")
    else:
    ...
    if mixup_active:
        ...
        if args.prefetcher and not _is_dvs_dataset(args.dataset):
            assert (
                not num_aug_splits
    ...
    if num_aug_splits > 1 and not _is_dvs_dataset(args.dataset):
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        input = input.float()
        if not args.prefetcher or _is_dvs_dataset(args.dataset):
