...
def random_spike_mask(x: torch.Tensor, mask_ratio: float) -> torch.Tensor:
    if mask_ratio is None or mask_ratio <= 0.0:
        return x
    if mask_ratio >= 1.0:
        return x.mul_(0.0)
...
class MS_MLP_Conv(nn.Module):
    ...
    def forward(self, x, hook=None):
        ...
        x = self.fc1_lif(x)
        x = random_spike_mask(x, self.rsm_ratio)
        if hook is not None:
...
class MS_SSA_Conv(nn.Module):
    ...
    def forward(self, x, hook=None):
        ...
        q_conv_out = random_spike_mask(q_conv_out, self.rsm_ratio)
        ...
        k_conv_out = random_spike_mask(k_conv_out, self.rsm_ratio)
        if self.dvs:
        ...
        v_conv_out = random_spike_mask(v_conv_out, self.rsm_ratio)
        if self.dvs:
        ...
        kv = random_spike_mask(kv, self.rsm_ratio)
        if hook is not None:
