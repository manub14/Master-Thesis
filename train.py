...
# DVS-specific augmentations for CIFAR10-DVS / CIFAR100-DVS
parser.add_argument("--dvs-timemask", action="store_true", default=False)
parser.add_argument("--dvs-timemask-p", type=float, default=0.5)
parser.add_argument("--dvs-timemask-max", type=int, default=1)

parser.add_argument("--dvs-timeshuffle", action="store_true", default=False)
parser.add_argument("--dvs-timeshuffle-p", type=float, default=0.5)

parser.add_argument("--dvs-patchdrop", action="store_true", default=False)
parser.add_argument("--dvs-patchdrop-p", type=float, default=0.5)
parser.add_argument("--dvs-patchdrop-ps", type=int, default=8)

parser.add_argument("--dvs-resize", type=int, default=64)
...
def is_dvs_dataset(name):
    return name in getattr(dvs_utils, "DVS_DATASET", []) or name in {
        "cifar100-dvs",
        "gesture",
    }
...
class SimpleDVSFrameFolder(torch.utils.data.Dataset):
    """
    Generic CIFAR100-DVS style loader.

    Expected structure:
        root/
          class0/
            sample1.npy / sample1.npz / sample1.pt
          class1/
            sample2.npy / sample2.npz / sample2.pt

    Each sample should be frame tensor:
        [T, C, H, W] or [C, T, H, W]
    """

    def __init__(self, root, frames_number=4, resize=64):
        self.root = root
        ...
        for c in self.classes:
            ...
            for ext in ("*.npy", "*.npz", "*.pt", "*.pth"):
                for path in glob.glob(os.path.join(cdir, ext)):
                    self.samples.append((path, self.class_to_idx[c]))

        if len(self.samples) == 0:
            raise RuntimeError(
                f"No DVS frame files found in {root}. "
                "Expected class folders containing .npy/.npz/.pt files."
            )
    ...
    def _load_file(self, path):
        ...
        # If [C, T, H, W], convert to [T, C, H, W]
        if x.ndim == 4 and x.shape[0] in (1, 2) and x.shape[1] == self.frames_number:
            x = x.permute(1, 0, 2, 3).contiguous()

        # Force time length
        if x.ndim == 4:
            T = x.shape[0]
            if T > self.frames_number:
                x = x[:self.frames_number]
            elif T < self.frames_number:
                pad = x[-1:].repeat(self.frames_number - T, 1, 1, 1)
                x = torch.cat([x, pad], dim=0)

        # Resize spatial size
        if x.ndim == 4 and self.resize is not None:
...
def dvs_time_mask(x, p=0.5, max_mask=1):
    # x: [B, T, C, H, W]
    if p <= 0 or torch.rand(1, device=x.device).item() > p:
        return x

    B, T, C, H, W = x.shape
    max_mask = max(1, min(max_mask, T))

    x = x.clone()
    for b in range(B):
        idx = torch.randperm(T, device=x.device)[:max_mask]
        x[b, idx] = 0.0

    return x


def dvs_time_shuffle(x, p=0.5):
    # x: [B, T, C, H, W]
    if p <= 0 or torch.rand(1, device=x.device).item() > p:
        return x
...
def dvs_patch_drop(x, p=0.5, ps=8):
    # x: [B, T, C, H, W]
    if p <= 0 or torch.rand(1, device=x.device).item() > p:
        return x
    ...
    x = x.clone()
    for b in range(B):
        h = torch.randint(0, H - ps + 1, (1,), device=x.device).item()
        w = torch.randint(0, W - ps + 1, (1,), device=x.device).item()
        x[b, :, :, h:h + ps, w:w + ps] = 0.0

    return x


def apply_dvs_augmentations(input, args):
    if args.dvs_timemask:
        input = dvs_time_mask(
            input,
            p=args.dvs_timemask_p,
            max_mask=args.dvs_timemask_max,
        )

    if args.dvs_timeshuffle:
        input = dvs_time_shuffle(
            input,
            p=args.dvs_timeshuffle_p,
        )

    if args.dvs_patchdrop:
        input = dvs_patch_drop(
            input,
            p=args.dvs_patchdrop_p,
            ps=args.dvs_patchdrop_ps,
        )
...
def main():
    ...
    args, args_text = _parse_args()
        # Tiny ImageNet handling
    # Tiny ImageNet handling
    args.is_tiny_imagenet = str(args.dataset).lower() in {
        "tiny-imagenet",
        "tiny_imagenet",
        "tinyimagenet",
    }

    if args.is_tiny_imagenet:
        ...
        _logger.info(
            "Auto-configured Tiny ImageNet: "
            f"num_classes={args.num_classes}, img_size={args.img_size}, "
            f"input_size={args.input_size}, val_split={args.val_split}"
        )
    if args.dataset == "gesture":
        ...
        _logger.info(
            "Auto-configured DVS128Gesture: "
            f"num_classes={args.num_classes}, in_channels={args.in_channels}, "
            f"img_size={args.img_size}, input_size={args.input_size}"
        )
    if args.dim is None:
        raise ValueError("Please provide --dim or define dim in the YAML config.")

    if args.img_size is None:
        raise ValueError("Please provide --img-size or define it in the YAML config.")
    ...
    args.dvs_mode = is_dvs_dataset(args.dataset)
    ...
    if args.dataset == "cifar10-dvs-tet":
        ...
        dataset = SimpleDVSFrameFolder(
            root=args.data_dir,
            frames_number=args.time_steps,
            resize=args.dvs_resize,
        )
    ...
    elif getattr(args, "is_tiny_imagenet", False):
    ...
    if mixup_active:
        ...
        if args.prefetcher and not is_dvs_dataset(args.dataset):
            assert (
                not num_aug_splits
    ...
    if num_aug_splits > 1 and not is_dvs_dataset(args.dataset):
    ...
    if is_dvs_dataset(args.dataset):
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        input = input.float()
        if not args.prefetcher or is_dvs_dataset(args.dataset):
            if args.amp and not isinstance(input, torch.cuda.HalfTensor):
                input = input.half()
            input, target = input.cuda(), target.cuda()
            if is_dvs_dataset(args.dataset):
                input = apply_dvs_augmentations(input, args)
